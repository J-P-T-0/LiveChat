...
public class ClientHandler extends Thread {
        ...
        try {
            ...
            while ((linea = entrada.readLine()) != null) {
                try {
                    ...
                    /*ObjectNode comando = (ObjectNode) objectMapper.readTree(linea);
    ...
    private void registrarUsuario(String telefono, String nombre, String password) throws SQLException {
        ...
        try (PreparedStatement checkStmt = conn.prepareStatement(checkTel)) {
        ...
        String sql = "INSERT INTO usuarios (telefono, nombre, contrasenia) VALUES (?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            if (resultado > 0) {
                enviarRespuesta("success", "Usuario registrado exitosamente");
            } else {
    ...
    private boolean autenticarUsuario(String telefono, String password) {
    ...
    private void registrar(ObjectNode comando) {
        try {
            String telefono = comando.get("telefono").asText();//comando de Json, cuando esta vacio retorna "" a diferencia de toString() que retorna null
            String nombre = comando.get("nombre").asText();
            String password = comando.get("password").asText();
            registrarUsuario(telefono, nombre, password);
            enviarRespuesta("success", "Usuario registrado correctamente");
        } catch (SQLException e) {
